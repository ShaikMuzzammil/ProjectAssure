...
// v22: compact INR for charts/cards — accepts rupees (not lakhs) and returns Cr/L shorthand.
export function inrCompact(value: number): string {
  if (value >= 1e7) return `₹${(value / 1e7).toFixed(2)} Cr`;
  if (value >= 1e5) return `₹${(value / 1e5).toFixed(1)} Cr`;
  if (value >= 1e3) return `₹${(value / 1e3).toFixed(1)} L`;
