...
// ─── v23: INPUT-DRIVEN RISK PREVIEW (the create-project wizard) ──────────────
// A deterministic, explainable risk estimate computed from EXACTLY what the
// user has typed so far — budget, timeline, team, contractor, documents.
// It updates live on every keystroke and reacts honestly to missing or
// unrealistic entries (empty fields RAISE risk; complete realistic plans
// lower it). The same signals then flow into the live register after create.
...
export interface RiskPreviewInput {
  ...
  documentCount: number;        // files staged in the wizard
  documentText?: string;        // concatenated text of staged documents
}

export interface RiskPreview {
  risks: { title: string; severity: "LOW" | "MEDIUM" | "HIGH" | "CRITICAL"; why: string; fix: string; points: number }[];
  overallRisk: number;          // 0-100
  riskLevel: RiskLevel;
  scheduleRisk: number;
  budgetRisk: number;
  resourceRisk: number;
  completeness: number;         // 0-100 — how complete are the inputs
  topFix: string;
}

export function predictRiskFromInputs(input: RiskPreviewInput): RiskPreview {
  const risks: RiskPreview["risks"] = [];
  const add = (title: string, severity: RiskPreview["risks"][number]["severity"], why: string, fix: string, points: number) =>
    risks.push({ title, severity, why, fix, points });

  // 1) completeness — missing fields are honest risk drivers
  const missing: string[] = [];
  if (!input.name.trim()) missing.push("project name");
  if (!input.state.trim()) missing.push("state");
  if (!input.district.trim()) missing.push("district");
  if (!input.totalBudget) missing.push("budget");
  if (!input.durationMonths) missing.push("duration");
  if (!input.startDate) missing.push("start date");
  const completeness = Math.max(0, 100 - missing.length * 14);
  if (missing.length >= 3) {
    add("Incomplete project details", "HIGH", `${missing.length} required fields are still empty (${missing.slice(0, 4).join(", ")}…). Predictions and approvals need the full picture.`, "Fill the remaining wizard fields — the risk score drops as data completes.", 52);
  } else if (missing.length > 0) {
    add("Some details still missing", "MEDIUM", `Not yet entered: ${missing.join(", ")}.`, "Complete these fields for an accurate baseline.", 34);
  }

  // 2) timeline sanity — duration vs budget pace
  const dur = input.durationMonths || 0;
  const budgetL = input.totalBudget || 0;
  if (dur > 0 && budgetL > 0) {
    const monthlyPaceL = budgetL / dur;                     // ₹ lakh / month
    const monthlyPaceCr = monthlyPaceL / 100;
    if (monthlyPaceCr > 120) {
      add("Extremely aggressive spending pace", "CRITICAL", `₹${budgetL / 100} Cr over ${dur} months means ₹${monthlyPaceCr.toFixed(0)} Cr/month — beyond what most executing agencies can absorb.`, "Extend the duration or phase the sanction; unrealistic pace is the #1 schedule-slip cause.", 78);
    } else if (monthlyPaceCr > 60) {
      add("Aggressive spending pace", "MEDIUM", `₹${monthlyPaceCr.toFixed(0)} Cr/month planned spend — high but workable with strong procurement.`, "Pre-award long-lead packages early to protect this pace.", 48);
    }
    if (dur < 8 && budgetL > 20000) {
      add("Very short timeline for the sanction size", "HIGH", `${dur} months for ₹${budgetL / 100} Cr leaves little float for weather or clearances.`, "Add buffer months for monsoon and statutory approvals.", 62);
    }
    if (dur > 48) {
      add("Very long duration window", "LOW", `${dur} months — price escalation and scope drift grow with duration.`, "Build an escalation clause and stage-gate reviews every 12 months.", 30);
    }
  }

  // 3) start/target consistency
  if (input.startDate && input.targetDate) {
    ...
    if (Number.isFinite(start) && Number.isFinite(target)) {
      const impliedMonths = Math.round((target - start) / (30.4 * 86400000));
      if (impliedMonths <= 0) {
        add("Target date is before the start date", "HIGH", "The timeline as entered cannot happen — the target precedes the start.", "Fix the dates: target must be after the start.", 70);
      } else if (dur > 0 && Math.abs(impliedMonths - dur) > 2) {
        add("Duration and dates disagree", "MEDIUM", `Dates imply ${impliedMonths} months but duration says ${dur} months.`, "Align the target date with the duration (±1 month).", 40);
      }
      const monthsToStart = (start - Date.now()) / (30.4 * 86400000);
      if (input.stage === "PLANNING" && monthsToStart < -6) {
        add("Planning stage but the start date has passed", "MEDIUM", `Start was ${Math.abs(Math.round(monthsToStart))} months ago while the project is still in Planning.`, "Either mark the stage as Active (execution) or revise the start date.", 44);
      }
    }
  }

  // 4) team & contractor
  if (input.teamSize > 0 && input.teamSize < 8 && budgetL > 5000) {
    add("Very thin team for the sanction", "MEDIUM", `${input.teamSize} people for ₹${budgetL / 100} Cr — supervision capacity is the constraint.`, "Plan a PMC or augment site supervision.", 42);
  }
  if (!input.teamSize) {
    add("Team size not specified", "LOW", "Resource planning needs a headcount to validate mobilisation.", "Enter the planned team size (default 12 applies otherwise).", 22);
  }
  if (!input.contractor.trim() || /tbd|pending|tender/i.test(input.contractor)) {
    add("Contractor not onboard", "MEDIUM", "No executing contractor is named — price and start date remain unknowns.", "Close the tender or record the intended contractor.", 46);
  }

  // 5) documents — the biggest lever the user controls
  if (input.documentCount === 0) {
    add("No documents uploaded yet", "MEDIUM", "DPR / clearance / progress documents drive the sharpest risk signals. Without them the register relies on plan-level heuristics only.", "Upload the DPR, tender or latest progress report in the Documents step.", 44);
  } else {
    const text = input.documentText ?? "";
    const docRisks = text ? scanDocumentRisks(text, "wizard upload").slice(0, 5) : [];
    for (const r of docRisks) {
      add(r.title, r.severity, r.evidence, r.mitigation, Math.round(r.impact * (0.5 + r.likelihood / 200)));
    }
    if (input.documentCount >= 3 && docRisks.length === 0) {
      add("Well-documented plan", "LOW", `${input.documentCount} documents staged and no material risk language detected — a strong start.`, "Keep monthly reports flowing after onboarding.", 12);
    }
  }

  // 6) sector & location context
  const sector = (input.sector || "").toLowerCase();
  const month = new Date().getMonth() + 1;
  if (/water|irrigation|river|drainage|sewer/.test(sector) && month >= 6 && month <= 9) {
    add("Monsoon window exposure", "HIGH", `${input.sector || "Water-sector"} works in ${input.state || "the chosen state"} — monsoon months (Jun–Sep) typically stall site work.`, "Schedule monsoon-unsafe activities outside the window.", 55);
  }
  if (/road|highway|transport|metro|rail/.test(sector)) {
    add("Right-of-way & utility shifting", "MEDIUM", "Linear corridor projects depend on land handover and utility agencies.", "Secure a joint utility plan with dates before execution.", 40);
  }
  if (budgetL >= 100000) {
    add("Large-project escalation rules", "LOW", `₹${budgetL / 100} Cr sanction crosses the Cabinet-level escalation line.`, "Track the 10% / 20% overrun bands actively.", 28);
  }

  // score: weighted points, completeness matters
  const weightSum = risks.reduce((s, r) => s + r.points, 0);
  const raw = risks.length ? Math.min(100, Math.round(weightSum / Math.sqrt(risks.length) * 0.62)) : 8;
  const overallRisk = Math.max(5, Math.round(raw * (0.55 + (100 - completeness) / 200)));
  const riskLevel: RiskLevel = overallRisk > 70 ? "CRITICAL" : overallRisk > 45 ? "HIGH" : overallRisk > 25 ? "MEDIUM" : "LOW";
  const scheduleRisk = Math.round(avgNum(risks.filter(r => /pace|timeline|dates|duration|monsoon|right-of-way/i.test(r.title)).map(r => r.points)));
  const budgetRisk = Math.round(avgNum(risks.filter(r => /pace|spend|budget|escalation|thin team/i.test(r.title + r.why)).map(r => r.points)));
  const resourceRisk = Math.round(avgNum(risks.filter(r => /team|contractor|mobilis/i.test(r.title)).map(r => r.points)));
  const topFix = risks.slice().sort((a, b) => b.points - a.points)[0]?.fix ?? "Inputs look complete and realistic — proceed to create.";

  return { risks: risks.sort((a, b) => b.points - a.points), overallRisk, riskLevel, scheduleRisk, budgetRisk, resourceRisk, completeness, topFix };
}

function avgNum(xs: number[]): number { return xs.length ? xs.reduce((a, b) => a + b, 0) / xs.length : 0; }
