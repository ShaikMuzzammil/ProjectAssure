...
export const DEFS: ProjDef[] = [
  // ─── the curated 5-project demo portfolio (complete, varied, jury-ready) ──
  ...
  { name: "Varanasi Sewage Treatment Plant (140 MLD)", sector: "Water", scheme: "Namami Gange", state: "Uttar Pradesh", district: "Varanasi", lat: 25.32, lng: 82.99, budgetL: 18600, dur: 22, startAgoM: 24, progress: 100, status: "COMPLETED", dept: "dept-soc", pm: "Ravi Menon", story: "The completed reference: on-time, on-budget delivery with full milestone closure — shows what a healthy end-state looks like in every view.", team: 64 },
];
...
export function buildWorld(): { projects: Project[]; globalAudit: AuditLogEntry[]; notifications: Notification[]; emails: EmailMessage[]; users: User[]; departments: Department[] } {
  ...
  const globalAudit: AuditLogEntry[] = [
    ...
    { id: "ga-2", action: "PREDICTION_RUN", entity: "CronJob", details: "6-hourly batch scoring: 4 ACTIVE projects scored · no level change", userName: "scheduler", userRole: "ADMIN", timestamp: shiftHours(new Date(), -3).toISOString() },
  ...
  const notifications: Notification[] = [
    ...
    { id: "nt-2", userId: "all", title: "Prediction run completed", message: "4 active projects re-scored · Bharatmala delay probability 75% (↑2 pts) · model AssurePredict 2.3.", type: "PREDICTION", isRead: false, createdAt: new Date(now.getTime() - 3 * 3600000).toISOString(), linkView: "ai-assistant" },
    ...
    { id: "nt-4", userId: "all", title: "Weekly digest delivered", message: "Portfolio pulse: 1 Healthy · 3 At-Risk · 1 Critical · ₹3.35k Cr sanctioned.", type: "EMAIL", isRead: true, createdAt: new Date(now.getTime() - 30 * 3600000).toISOString(), linkView: "email-center" },
    { id: "nt-5", userId: "u-sec", title: "Welcome to ProjectAssure", message: "Your Portfolio Overseer console is ready. 5 demo projects are being monitored. Try ⌘K.", type: "SYSTEM", isRead: true, createdAt: shiftDays(ANCHOR, -30).toISOString() },
  ];
  ...
  const emails: EmailMessage[] = [
    ...
    { id: "em-2", to: "arun.kulkarni@mospi.gov.in", subject: "Weekly Portfolio Digest — 5 projects", body: "Portfolio pulse: 1/3/1...", template: "weekly_digest", status: "SENT", createdAt: new Date(now.getTime() - 30 * 3600000).toISOString(), sentAt: new Date(now.getTime() - 30 * 3600000).toISOString(), provider: "smtp-gmail", attachments: [{ name: "portfolio-digest-2026-09-07.pdf", kind: "pdf", sizeKb: 312 }], replyTo: "noreply@projectassure.in" },
